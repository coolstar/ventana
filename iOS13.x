...
static CSCoverSheetViewController *MLS_dashBoardViewController;
...
+ (instancetype)lastInstance {
	return (NCNotificationStructuredListViewController *)LastPriorityControllerInstance;
}
...
- (void)showMetroLockScreen {
	NCNotificationCombinedListViewController *priorityController = [self valueForKey:@"_structuredListViewController"];
	LastPriorityControllerInstance = priorityController;

	CSMetroLockScreenViewController *controller = [CSMetroLockScreenViewController sharedLockScreenController];
	[controller gotNativeNotificationController:priorityController];
	controller.nativeController13 = self;
}
...
- (void)hideMetroLockScreen {
	...
	controller.nativeController13 = nil;
...
%hook CSPresentationViewController
- (void)presentContentViewControllers:(NSArray *)contentControllers animated:(BOOL)animated completion:(id)completion {
	...
	if (enabled && MLS_UIisReallyLocked){
		...
		for (NSObject *controller in contentControllers){
			...
			if ([controller isKindOfClass:objc_getClass("CSMediaControlsViewController")])
				continue;
			if ([controller isKindOfClass:objc_getClass("CSNowPlayingViewController")])
				continue;
...
- (void)setPasscodeLockVisible:(BOOL)visible animated:(_Bool)animated completion:(id)arg3 {
	...
	if (enabled && MLS_UIisReallyLocked){
		[UIView animateWithDuration:animated ? 0.25 : 0.0 animations:^{
			CGFloat targetX = [[self coverSheetView] scrollView].contentOffset.x;

			CGFloat backgroundChange = ((targetX - [self coverSheetView].bounds.size.width)/[self coverSheetView].bounds.size.width);
...
- (void)displayMetroLockScreen {
	...
	if (enabled){
		...
		CGRect frame = [[self coverSheetView] bounds];
		frame.origin.x = frame.size.width * [self _indexOfMainPage];
		...
		[[[self coverSheetView] scrollView] addSubview:controller.view];
		[[[self coverSheetView] mainPageView] setAlpha:0];
		[[[self coverSheetView] fixedFooterView] setAlpha:0];
		[[[self coverSheetView] quickActionsView] setAlpha:0];
		[[[self coverSheetView] teachableMomentsContainerView] setAlpha:0];
		[[self coverSheetView] updateBackgroundShade];
...
- (void)hideMetroLockScreen {
	[UIView animateWithDuration:0.25 animations:^{
		...
		[[[self coverSheetView] mainPageView] setAlpha:1];
		[[[self coverSheetView] fixedFooterView] setAlpha:1];
		[[[self coverSheetView] quickActionsView] setAlpha:1];
		[[[self coverSheetView] teachableMomentsContainerView] setAlpha:1];
		[[self coverSheetView] updateBackgroundShade];
...
- (void)resetLockScreenIdleTimer {
	SBDashBoardIdleTimerProvider *idleTimeProvider = [self valueForKey:@"_idleTimerController"];
	[idleTimeProvider resetIdleTimerIfTopMost];
}
...
- (void)_updateQuickActions {
	...
	BOOL enabled = [[CSMetroLockScreenSettingsManager sharedInstance] enabled];
	if (enabled && MLS_UIisReallyLocked){
		[[[self coverSheetView] quickActionsView] setAlpha:0.0f];
	}
}
...
- (void)CSMLS_updateStatusBarAlpha {
	...
	if (enabled && MLS_UIisReallyLocked){
		...
		CGFloat backgroundChange = ((targetX - ([self coverSheetView].bounds.size.width * [self _indexOfMainPage]))/[self coverSheetView].bounds.size.width);
